...
function Teacher() {
  ...
  const handleConfirm = () => {
      if (selectedUserId) {
        axios
          .delete(`${Api_url}/user/${selectedUserId}`)
          .then((response) => {
  ...
  const handleEditProfile = (user) => {
    ...
    setEditJoiningdate(user.joiningdate.split("T")[0]);
...
console.log(updatedUser);
  ...
  return (
    <div>
      ...
      <div className='flex justify-between mb-8'>
        <div>
          <h2 className='text-xl'>Teacher List</h2>
          <h4 className='text-slate-600 text-[0.8rem]'>ADMIN SETTING</h4>
        </div>
        <button
          onClick={() => navigate('/Accueil/Teacher/Add')}
      ...
      {showProfileModal && !showEditProfileModal && (
        <div className='fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-80'>
          <div className='bg-white w-auto lg:w-auto top-[3.7rem] lg:top-8 p-4 rounded-lg relative lg:left-32'>
  <MdClose
  ...
  <div className='grid grid-cols-1 gap-4 mb-4'>
    ...
    <div className='flex'>
      ...
      <p className='text-lg ml-14 text-right'>{selectedUser.email}</p>
    </div>
    <div className='flex'>
      <p className='text-lg font-semibold mb-2 flex-grow'>Phone:</p>
      <p className='text-lg ml-4 text-right'>{selectedUser.phone}</p>
    </div>
    <div className='flex'>
    ...
    <div className='flex'>
      <p className='text-lg font-semibold mb-2 flex-grow'>Date of Joining:</p>
      ...
      {showEditProfileModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-80">
        <div className="bg-white lg:left-32 top-6 w-[22rem] lg:w-[28rem] md:w-[28rem] p-6 rounded-lg relative">
          ...
          <form className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col">
                <label htmlFor="joiningdate" className="text-lg font-semibold mb-2">
                Joining Date
                </label>
                <input
                  id="joiningdate"
        ...
        <div className="inline-flex items-center ml-4 lg:ml-6 mt-2">
          <input
            className="form-radio h-5 w-5 text-gray-600"
            type="radio"
            name="gender"
